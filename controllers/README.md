...
| Software dispatcher (1 bootstrap Start, then SMB-fed jobs) | ✅ `[CONFIRMED]` `M47` loop + overwrite file; no per-job trigger | ⚠️ **Expert `M47` ≠ V4.1 `M47`** (here it's a count/pause macro, `O10047`) — V4.1 loop trick does NOT port; need `M99`/`M98` loop `[TO TEST 2026-06-06]`. `sysstart` boot-run = zero-touch bootstrap ✅ |
